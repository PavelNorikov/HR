...
using static ConsoleApp1.EF.MyCont;
...
static async void MyEF()
{
    ...
    Console.WriteLine("Вывести список соискателей, отсортированных по названию вакансии, внутри вакансии по зарплате.");
    foreach (cont_applicant a in db.cont_applicants.OrderByDescending(a => a.Vacancy.Title).ThenBy(a => a.Salary))
    {
        Console.WriteLine($"{a.Name} {a.Vacancy.Title} {a.Salary}");
    }

    Console.WriteLine("\n\n Вывести первых двух наиболее привлекательных соискателей программистов, (привлекательность считать как стаж, делённый на зарплатные ожидания)");
    foreach (cont_applicant a in db.cont_applicants.Where(a => a.Vacancy.Title == "Программист").OrderByDescending(a => a.Exp / a.Salary).Take(2))
    {
        Console.WriteLine($"{a.Name}");
    }

    Console.WriteLine("\n\n Вывести список соискателей, с зарплатными ожиданиями “выше среднего по базе”, вместе с их специальностями.");
    foreach (var a in db.cont_applicants.Where(a => a.Salary > db.cont_applicants.Average(a => a.Salary)))
    {
        Console.WriteLine($"{a.Name} {a.Vacancy.Title} {a.Salary}");
    }

    Console.WriteLine("\n\n Вывести пары: {специальность, среднее зарплатное ожидание}, отсортировать от более высокооплачиваемых специальностей к менее.");
    foreach (var g in db.cont_vacancies.Select(x => new { Title = x.Title, avg = db.cont_applicants.Where(a => a.Vacancy.Id == x.Id).Average(a => a.Salary) }).OrderByDescending(t => t.avg))
    {
        Console.WriteLine($"{g.Title} {g.avg}");
    }
    ...
    Console.WriteLine("\n\n Взять кандидатов с наименьшем стажем в каждой из специальностей – удалить из базы.");
    Console.WriteLine("До:");
    foreach (cont_applicant a in db.cont_applicants)
    {
        Console.WriteLine($"{a.Name} {a.Exp} {a.Vacancy.Title}");
    }
    List<cont_applicant> res;
    res = new List<cont_applicant> { };
    Console.WriteLine("");
    foreach (var g in db.cont_vacancies.Select(x => new { Title = x.Title, Min = db.cont_applicants.Where(a => a.Vacancy.Id == x.Id).Min(a => a.Exp) }))
    {
        MyCont db2 = new MyCont();
        foreach (var p in db2.cont_applicants.Where(p => p.Vacancy.Title == g.Title && p.Exp == g.Min))
        {
            res.Add(p);
        }
        db2.Dispose();
    ...
    MyCont db9 = new MyCont();

    foreach (cont_applicant t in res)
    {
        db9.cont_applicants.Remove(t);
    }
    db9.SaveChanges();
    db.SaveChanges();
    ...
    Console.WriteLine("");
    foreach (var g in db.cont_applicants)
    {
        MyCont db2 = new MyCont();
        if (g.Salary == db2.cont_applicants.Max(g => g.Salary))
        {
            g.Salary = g.Salary * Convert.ToDecimal(0.85);
            break;
        }
    ...
    foreach (cont_applicant a in db.cont_applicants)
    {
        Console.WriteLine($"{a.Name} {a.Vacancy.Title}");
    }
    res = new List<cont_applicant> { };
    Console.WriteLine("");


    cont_vacancy res_add = db.cont_vacancies.First(t => t.Id == 1);
    foreach (var g in db.cont_vacancies.Select(x => new { vacan = x, count = db.cont_applicants.Where(a => a.Vacancy.Id == x.Id).Count() }).OrderBy(c => c.count).Take(1))
        {
        res_add = g.vacan;
        break;
    }
    db.cont_applicants.Add(new cont_applicant { Name = "Тест Тестов Тестович", Exp = 99, Salary = 9999999, Vacancy = res_add });
    db.SaveChanges();
    Console.WriteLine("После:");
    foreach (cont_applicant a in db.cont_applicants)
    {
